...
model = skip_ns(nwords, EMB_SIZE).cuda()
...
def calc_sent_loss(sent):
    ...
    for i, word in enumerate(sent):
        ...
        pos_words_tensor = torch.cuda.LongTensor(pos_words)
        neg_words = all_neg_words[i*K*2*N:(i+1)*K*2*N]
        neg_words_tensor = torch.cuda.LongTensor(neg_words)
        target_word_tensor = torch.cuda.LongTensor([word])
